...
class IgrejaRenovadaSupabaseUser extends BaseAuthUser {
  IgrejaRenovadaSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> igrejaRenovadaSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = IgrejaRenovadaSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
